...
LLM Client ──MCP──► Grok Search Server (Go)
                      ├─ web_search  ───► Grok 端点池（按优先级降级）→ Exa Search → Tavily Search 兑底
                      ├─ web_fetch   ───► Jina Reader → Exa Contents → Tavily Extract（降级抓取）
...
| 命令 | 引擎 | 主要 flags |
|------|------|-----------|
| `search <query>` | Grok 池 → Exa → Tavily 兑底 | `--platform` `--model` `--timeout` `--json` |
| `fetch <url>` | Jina Reader → Exa Contents → Tavily Extract | `--timeout` `--json` |
