...
| 命令 | 引擎 | 主要 flags |
|------|------|-----------|
| `search <query>` | Grok 池 → Tavily 兑底 | `--platform` `--model` `--timeout` `--json` |
| `fetch <url>` | Jina Reader → Tavily Extract | `--timeout` `--json` |
...
| 工具 | 描述 |
|------|------|
| `web_search` | AI 驱动的网络搜索（Grok 池 → Tavily Search 兑底）。支持按次传 `model` 覆盖默认模型；返回 `engine: <name> (<model>)` + `session_id` + 答案 |
