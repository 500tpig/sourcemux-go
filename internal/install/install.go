...
func ApplyPlan(plan Plan, opts options) error {
	for i := range plan.Actions {
		...
		switch action.Type {
		case "write_file":
			content := routingSkill(plan.Binary, plan.ConfigFile)
			status, backup, err := writeFile(action.Path, []byte(content), opts.Force)
			if err != nil {
				return err
			}
		...
		case "remove_file":
			status, err := removeFile(action.Path)
			if err != nil {
				return err
			}
			action.Status = status
...
func addUninstallActions(plan *Plan, target Target, scope string) {
	...
	plan.Actions = append(plan.Actions, PlanAction{
		...
		Path:    path,
		Status:  "planned",
		Message: "Remove SourceMux routing skill file.",
	})
...
func writeFile(path string, content []byte, force bool) (string, string, error) {
